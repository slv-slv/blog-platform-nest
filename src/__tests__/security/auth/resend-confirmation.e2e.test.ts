...
describe('RESEND CONFIRMATION', () => {
  ...
  it('should resend code for not confirmed user', async () => {
    ...
    const beforeResendResult = await pool.query(`SELECT confirmation_code FROM users WHERE email = $1`, [
      user.email,
    ]);
    const beforeResend = beforeResendResult.rows[0] ?? null;
    ...
    const afterResendResult = await pool.query(
      `
        SELECT is_confirmed, confirmation_code
        FROM users
        WHERE email = $1
      `,
      [user.email],
    );
    const afterResend = afterResendResult.rows[0] ?? null;
    expect(afterResend?.is_confirmed).toBe(false);
    expect(afterResend?.confirmation_code).toBeTruthy();
    expect(afterResend?.confirmation_code).not.toBe(beforeResend?.confirmation_code);
  });
});
