...
describe('UPDATE POST', () => {
  ...
  it('should return 204 and update post', async () => {
    ...
    expect(response.body).toHaveProperty('shortDescription', 'Updated short description');
    expect(response.body).toHaveProperty('content', 'Updated long enough post content');
  });
  ...
  it('should return 404 if blog does not exist', async () => {
    ...
    await request(httpServer)
      .put(`/sa/blogs/999999/posts/${postId}`)
      .set('Authorization', adminAuthHeader)
      .send({
        title: 'Updated title',
        shortDescription: 'Updated short description',
  ...
  it('should return 404 if post does not exist', async () => {
    ...
    await request(httpServer)
      .put(`/sa/blogs/${blogId}/posts/999999`)
      .set('Authorization', adminAuthHeader)
      .send({
        title: 'Updated title',
        shortDescription: 'Updated short description',
  ...
  it('should return 400 if body has incorrect values', async () => {
    ...
    expect(response.body.errorsMessages[0]).toHaveProperty('field', 'title');
  });
});
