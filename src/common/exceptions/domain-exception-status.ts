export enum DomainExceptionStatus {
  // Blog content
  BLOG_NOT_FOUND = 'BLOG_NOT_FOUND',
  POST_NOT_FOUND = 'POST_NOT_FOUND',
  COMMENT_NOT_FOUND = 'COMMENT_NOT_FOUND',

  // Security / sessions
  DEVICE_NOT_FOUND = 'DEVICE_NOT_FOUND',
  SESSION_NOT_ACTIVE = 'SESSION_NOT_ACTIVE',
  USER_ALREADY_LOGGED_IN = 'USER_ALREADY_LOGGED_IN',

  // Users
  USER_NOT_FOUND = 'USER_NOT_FOUND',
  LOGIN_ALREADY_EXISTS = 'LOGIN_ALREADY_EXISTS',
  EMAIL_ALREADY_EXISTS = 'EMAIL_ALREADY_EXISTS',
  INCORRECT_EMAIL = 'INCORRECT_EMAIL',

  // Quiz
  QUESTION_NOT_FOUND = 'QUESTION_NOT_FOUND',
  GAME_NOT_FOUND = 'GAME_NOT_FOUND',
  NO_ACTIVE_PAIR = 'NO_ACTIVE_PAIR',
  NO_REMAINING_QUESTIONS = 'NO_REMAINING_QUESTIONS',
  GAME_DEADLINE_EXPIRED = 'GAME_DEADLINE_EXPIRED',
  CANNOT_JOIN_OWN_GAME = 'CANNOT_JOIN_OWN_GAME',
  GAME_IS_NOT_PENDING = 'GAME_IS_NOT_PENDING',
  SECOND_PLAYER_ALREADY_JOINED = 'SECOND_PLAYER_ALREADY_JOINED',
  NOT_ENOUGH_QUESTIONS_TO_START_GAME = 'NOT_ENOUGH_QUESTIONS_TO_START_GAME',

  // Auth
  CREDENTIALS_INCORRECT = 'CREDENTIALS_INCORRECT',
  UNAUTHORIZED = 'UNAUTHORIZED',
  EMAIL_NOT_CONFIRMED = 'EMAIL_NOT_CONFIRMED',
  EMAIL_ALREADY_CONFIRMED = 'EMAIL_ALREADY_CONFIRMED',
  CONFIRMATION_CODE_INVALID = 'CONFIRMATION_CODE_INVALID',
  CONFIRMATION_CODE_EXPIRED = 'CONFIRMATION_CODE_EXPIRED',
  RECOVERY_CODE_INVALID = 'RECOVERY_CODE_INVALID',
  RECOVERY_CODE_EXPIRED = 'RECOVERY_CODE_EXPIRED',

  // Access
  ACCESS_DENIED = 'ACCESS_DENIED',
}
